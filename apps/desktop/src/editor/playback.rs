                ...
                for f_idx in 0..num_frames {
                    ...
                    for track in tracks.iter() {
                        ...
                        for clip in &track.clips {
                            ...
                            if !is_text && t_abs >= clip.start && t_abs <= clip.start + clip.duration {
...
mod tests {
    ...
    async fn test_command_undo_redo(cx: &mut gpui::TestAppContext) {
        ...
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips.len()), 2);

        // 2. Execute AddClipCommand
        let _ = core.update(cx, |core: &mut EditorCore, cx: &mut gpui::Context<EditorCore>| {
            core.execute_command(
                Box::new(AddClipCommand::new("TestClip.mp4".to_string(), "".to_string(), 10.0, 5.0, "#000000".to_string(), 0)),
                cx,
            );
        });
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips.len()), 3);
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips[2].name.clone()), "TestClip.mp4");
    ...
    async fn test_clip_edit_and_split(cx: &mut gpui::TestAppContext) {
        ...
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips[0].start), new_start);
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips[0].duration), new_duration);

        // Undo Edit
        let _ = core.update(cx, |core: &mut EditorCore, cx: &mut gpui::Context<EditorCore>| {
            core.undo(cx);
        });
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips[0].start), clip0_start);
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips[0].duration), clip0_duration);
        ...
        assert_eq!(split_clip.duration, clip0_duration - (split_playhead - clip0_start));
        ...
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips.len()), 3);
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips[0].duration), split_playhead - clip0_start);
        assert_eq!(cx.read(|cx| core.read(cx).timeline.tracks[0].clips[1].duration), clip0_duration - (split_playhead - clip0_start));
    }
    ...
    async fn test_clip_text_command(cx: &mut gpui::TestAppContext) {
        ...
        // Insert clip-text-1 dynamically
        ...
        assert_eq!(initial_clip.id, clip_id);
        assert_eq!(initial_clip.text_content, Some("歡迎使用 OpenCut 本地剪輯器！".to_string()));
        ...
        assert_eq!(reverted.text_content, Some("歡迎使用 OpenCut 本地剪輯器！".to_string()));
        ...
        let _ = core.update(cx, |core: &mut EditorCore, cx: &mut gpui::Context<EditorCore>| {
            core.execute_command(
                Box::new(EditClipAudioCommand::new(
                    clip_id.clone(),
                    initial_clip.volume, Some(0.5),
                    initial_clip.fade_in, Some(1.5),
                    initial_clip.fade_out, Some(2.0),
                )),
                cx,
            );
        });
